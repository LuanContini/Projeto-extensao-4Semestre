...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const tableRows = document.querySelectorAll(".grupos-table tbody tr");
  ...
  searchInput.addEventListener("input", function () {
    ...
    tableRows.forEach((row) => {
      ...
      const categoriaCell = row.querySelector("td:nth-child(3)");
      const idCell = row.querySelector("td:nth-child(1) img"); 

      const nomeValue = nomeCell.textContent.toLowerCase();
      const categoriaValue = categoriaCell.textContent.toLowerCase();

      if (
        nomeValue.includes(searchTerm) ||
        categoriaValue.includes(searchTerm)
      ) {
...
function sortTable(columnIndex) {
  ...
  rows.sort((rowA, rowB) => {
    ...
    if (columnIndex === 7) {
      ...
      // Números (Disponível, Reservado, Manutenção, Total)
...
function confirmDeletion(row, idGrupo) {
  ...
  message.innerHTML =
    '<strong style="color: red;">Tem certeza?</strong><br>Isso irá remover todos os itens adicionados ao produto.';
